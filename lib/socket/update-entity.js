    ...
    Object.keys(socketMap).forEach(function (socketId) {
      ...
      if (socketEntity) {
        ...
        if ('' + entityId === '' + socketEntityId) {
          ...
          let theObject = entity

          Object.keys(socketEntity['$modelOptions'].getterMethods).map(key => {
            console.log('the key', key)
            theObject = objectWithoutKey(theObject, key)
          })
            console.log('the object', theObject)

          console.log('socket entity', socketEntity['$modelOptions'].getterMethods)
